...
import { gt as gt_, lt as lt_, reorderInsert, resolveDimension } from '../../../../../utils';
...
const useInsertStrategy = () => {
  ...
  const swappedBeforeIndexes = useMutableValue(null);
  const swappedAfterIndexes = useMutableValue(null);
  const swappedBeforeLayout = useMutableValue(null);
  const swappedAfterLayout = useMutableValue(null);
  ...
  useAnimatedReaction(() => activeItemKey.value !== null && activeGroupIndex.value !== null && appliedLayout.value !== null ? {
    ...
    if (swappedBeforeIndexes.value) {
      swappedBeforeLayout.value = calculateFlexLayout(swappedBeforeIndexes.value.indexToKey);
    }
    if (swappedAfterIndexes.value) {
      swappedAfterLayout.value = calculateFlexLayout(swappedAfterIndexes.value.indexToKey);
    }
  });
  return ({
    activeIndex,
    ...
    do {
      ...
      if (groupIndex === 0) {
        ...
        const additionalSwapOffset = getAdditionalSwapOffset(beforeLayout?.crossAxisGroupSizes?.[beforeIndexes.groupIndex] ?? 0);
    ...
    do {
      ...
      const additionalSwapOffset = getAdditionalSwapOffset(afterLayout?.crossAxisGroupSizes?.[afterIndexes.groupIndex] ?? 0);
    ...
    if (!currentGroup) {
      ...
      return reorderInsert(indexToKey.value, activeIndex, lastItemIndex, fixedItemKeys?.value);
    }
    ...
    do {
      ...
      const itemMainSize = resolveDimension(mainItemSizes.value, currentItemKey);
      if (!currentItemPosition || itemMainSize === null) return;
      swapItemAfterOffset = currentItemPosition[mainCoordinate] + (isReverse ? 0 : itemMainSize);
      ...
      const nextItemMainSize = resolveDimension(mainItemSizes.value, nextItemKey);
    ...
    if (groupBefore && itemIndexInGroup > 0) {
      const groupBeforeSize = getTotalGroupSize(groupBefore, mainItemSizes.value, mainGap);
      canBeFirst = groupBeforeSize + activeItemMainSize + mainGap > currentLayout.groupSizeLimit;
    }
    ...
    do {
      ...
      const currentItemKey = currentGroup[itemIndexInGroup];
      const currentItemPosition = currentLayout.itemPositions[currentItemKey];
      const currentItemMainSize = resolveDimension(mainItemSizes.value, currentItemKey);
      if (!currentItemPosition || currentItemMainSize === null) return;
      swapItemBeforeOffset = currentItemPosition[mainCoordinate] + (isReverse ? currentItemMainSize : 0);
      ...
      const prevItemMainSize = resolveDimension(mainItemSizes.value, prevItemKey);
      ...
      const sizeToAdd = isPrevBeforeCurrent ? currentItemMainSize : prevItemMainSize;
    ...
    const newIndex = firstAvailableInGroupIndex + (itemIndexInGroup - initialItemIndexInGroup);
    if (newIndex === activeIndex || fixedItemKeys?.value?.[indexToKey.value[newIndex]]) {
      return;
    }
    return reorderInsert(indexToKey.value, activeIndex, newIndex, fixedItemKeys?.value);
  };
};
