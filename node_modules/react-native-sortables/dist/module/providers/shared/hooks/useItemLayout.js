...
import { interpolate, makeMutable, useAnimatedReaction, useDerivedValue, withTiming } from 'react-native-reanimated';
import { HIDDEN_X_OFFSET, IS_WEB, isFabric } from '../../../constants';
import { interpolateVector, useAnimatedDebounce, useMutableValue } from '../../../integrations/reanimated';
import { areVectorsDifferent } from '../../../utils';
import { useCommonValuesContext } from '../CommonValuesProvider';
import useItemZIndex from './useItemZIndex';
const RELATIVE_STYLE = {
  position: 'relative'
};
const HIDDEN_STYLE = {
 ...
 */
function useItemLayoutPaper(position, zIndex) {
  const {
    usesAbsoluteLayout
 ...
 */
function useItemLayoutFabric(position, zIndex) {
  const {
    activeItemDropped,
    usesAbsoluteLayout
  } = useCommonValuesContext();
  const transformStartPosition = useMutableValue(null);
...
export default function useItemLayout(key, isActive, activationAnimationProgress) {
  const {
    activeItemKey,
    activeItemPosition,
  ...
  const layoutPosition = useDerivedValue(() => itemPositions.value[key] ?? null);
  const positionRef = useRef(null);
  const interpolationStartValues = useMutableValue(null);
  positionRef.current ??= makeMutable(isActive.value ? activeItemPosition.value : layoutPosition.value);
  ...
  useAnimatedReaction(() => ({
    ...
    const positionChanged = prev?.itemPosition && areVectorsDifferent(prev.itemPosition, itemPosition, 1);
    if (progress === 0) {
    ...
    else if (interpolationStartValues.value ? positionChanged : prev?.active) {
    ...
    if (interpolationStartValues.value) {
      ...
      const currentProgress = interpolate(progress, [startProgress, 0], [0, 1]);
      position.value = interpolateVector(currentProgress, startPosition, itemPosition);
      return;
    }
    if (!positionChanged) {
      return;
    }
    if (shouldAnimateLayout.value && (!animateLayoutOnReorderOnly.value || activeItemKey.value !== null)) {
...
//# sourceMappingURL=useItemLayout.js.map