...
const MOCK_RESPONSES: Record<string, (context: { activeTour?: { name: string; artist: string }; nextShow?: { venue: string; city: string; state: string; date: string; loadIn: string; soundcheck: string; doors: string; showTime: string; curfew: string; capacity?: number; venueContact?: string; venuePhone?: string }; gear: { name: string; condition: string; notes?: string }[]; inputList: { channel: number; source: string; mic: string }[] }) => string> = {
  'day_sheet': (ctx) => {
    const show = ctx.nextShow;
    const tour = ctx.activeTour;
    if (!show || !tour) return 'No upcoming shows found. Create a tour and add shows to generate day sheets.';
    ...
    return `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
DAY SHEET
${tour.artist.toUpperCase()} • ${tour.name}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
${show.capacity ? `Capacity: ${show.capacity.toLocaleString()}` : ''}

TIMELINE (Local Time)
Load In ........... ${show.loadIn} – Crew call
Soundcheck ........ ${show.soundcheck} – Full band
Doors ............. ${show.doors}
Show .............. ${show.showTime}
Curfew ............ ${show.curfew}

VENUE CONTACTS
${show.venueContact ? `• ${show.venueContact} (Production): ${show.venuePhone || 'TBD'}` : '• Contact: TBD'}

PRODUCTION NOTES
• Confirm load-in access and dock availability
• Review house PA specs and monitor package
• Check dressing room and catering setup
...
Generated by Tour Flow`;
  ...
  'advance': (ctx) => {
    const show = ctx.nextShow;
    const tour = ctx.activeTour;
    if (!show || !tour) return 'No upcoming shows found. Create a tour and add shows to generate advance sheets.';

    return `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ADVANCE SHEET
${tour.artist} • ${show.venue}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
FOH Requirements:
• Digital console with minimum 32 inputs
• Full PA coverage for venue capacity
• Sub-bass reinforcement
• Delay speakers if venue depth exceeds 100ft

Monitor Requirements:
• Minimum 6 monitor mixes
• Drum fill required
• Side fills for stage coverage

INPUT LIST SUMMARY
${ctx.inputList.length > 0 ? `Total Channels: ${ctx.inputList.length}` : 'Input list pending'}

QUESTIONS FOR VENUE PM
□ Confirm PA specs and coverage
□ Monitor console availability
□ Power requirements and distro location
□ Dressing room and production office location
□ Catering delivery time
...
Generated by Tour Flow`;
  ...
  'settlement': (ctx) => {
    const show = ctx.nextShow;
    const tour = ctx.activeTour;
    if (!show || !tour) return 'No upcoming shows found. Create a tour and add shows to generate settlement sheets.';
...
Date: ${show.date}
Venue: ${show.venue}, ${show.city}, ${show.state}

GROSS RECEIPTS
Ticket Sales .............. $__________
VIP Packages .............. $__________
                            ──────────
Gross Revenue ............. $__________

DEDUCTIONS
Facility Fee .............. $__________
Ticket Commission ......... $__________
Credit Card Fees .......... $__________
Insurance ................. $__________
                            ──────────
Total Deductions .......... $__________

ADJUSTED GROSS ............ $__________

ARTIST SETTLEMENT
Guarantee ................. $__________
Bonus (after breakeven) ... $__________
                            ──────────
Artist Payment ............ $__________

ADDITIONAL INCOME
Merchandise ............... $__________
Less: Venue Commission .... $__________
Net Merchandise ........... $__________

TOTAL ARTIST .............. $__________
...
Generated by Tour Flow`;
  ...
  'mixing_tips': (ctx) => {
    const show = ctx.nextShow;
    return `MIXING TIPS FOR LIVE SOUND

${show ? `Venue: ${show.venue}, ${show.city}` : 'General venue tips'}

PRE-SHOW CHECKLIST
• Walk the room during soundcheck - check coverage in all sections
• Use pink noise to identify problem frequencies
• Set limiters on outputs before pushing level

FREQUENCY MANAGEMENT
• High-pass vocals around 80-100Hz to clean up mud
• Notch out 200-300Hz on guitars if fighting with vocals
• Be careful with 2-4kHz - ear fatigue zone

DYNAMICS
• Use gentle compression on vocals (3:1, medium attack)
• Parallel compression on drums for punch without squashing
• Leave headroom - don't ride the limiters

STAGE VOLUME
• Work with band on monitor levels before pushing FOH
• Use drum shield if kit is overpowering the PA
• IEMs help control stage volume significantly

FEEDBACK PREVENTION
• Ring out monitors before soundcheck
• Use graphic EQ surgically, not broadly
• Watch mic positioning relative to wedges

Generated by Tour Flow`;
  },

  'rf_coord': () => {
    return `RF COORDINATION GUIDE

FREQUENCY PLANNING

For PSM1000 IEMs:
• Use frequencies in the 470-608 MHz range (post-repack safe)
• Maintain minimum 300kHz spacing between IEM frequencies
• Keep IEMs at least 1MHz away from wireless mics

For ULXD Wireless Mics:
• Run Wireless Workbench scan before each show
• Group compatible frequencies together
• Avoid local TV broadcast frequencies

INTERMOD CALCULATION
• Calculate 3rd order intermod products
• Avoid: 2A-B, 2B-A, A+B-C type combinations
• Use Shure WWB or IAS frequency coordination software

ANTENNA PLACEMENT
• Position antennas above crowd level
• Use directional paddles facing stage
• Keep minimum 3ft between transmitters and receivers

BATTERY MANAGEMENT
• Use fresh batteries for each show
• Rechargeable: charge between shows, never partial charge
• Label batteries with cycle count

TROUBLESHOOTING DROPOUTS
• Check antenna connections
• Scan for new interference
• Adjust squelch if needed
• Consider diversity antenna combiners

Generated by Tour Flow`;
  },

  'gear_check': (ctx) => {
    const issueGear = ctx.gear.filter(g => g.condition === 'needs_repair' || g.condition === 'fair');

    return `PRE-TOUR MAINTENANCE CHECKLIST

${issueGear.length > 0 ? `⚠️ ITEMS NEEDING ATTENTION (${issueGear.length})
${issueGear.map(g => `• ${g.name} - ${g.condition.replace('_', ' ')}${g.notes ? `: ${g.notes}` : ''}`).join('\n')}

` : ''}AUDIO EQUIPMENT
□ Test all microphones with consistent signal
□ Check XLR cables for continuity and shorts
□ Verify phantom power operation
□ Test DI boxes with battery and phantom
□ Clean and inspect all connectors

WIRELESS SYSTEMS
□ Update firmware on all wireless units
□ Test RF link quality at distance
□ Verify battery contacts are clean
□ Check antenna connections
□ Run frequency scan in multiple venues

MONITORS
□ Test all IEM packs
□ Verify earpiece drivers
□ Check body pack belt clips
□ Test backup packs

CABLES & CONNECTIVITY
□ Test all multicore/snake channels
□ Verify digital snake protocol negotiation
□ Check network cables (if using Dante/AVB)
□ Test all AC power cables

DOCUMENTATION
□ Update input list with any changes
□ Confirm rider is current
□ Print emergency contact list
□ Backup console show files

Generated by Tour Flow`;
  },

  'parse_input': () => {
    return `Ready to parse your input list!

Please paste your input list text below. I'll organize it into a clean table format.

SUPPORTED FORMATS:
• Plain text lists (e.g., "1. Kick - Beta 52")
...
02 | Kick Out    | Beta 52     |
03 | Snare Top   | SM57        |
04 | Snare Bot   | SM57        | Phase flip

Paste your input list and I'll format it!`;
  },

  'parse_rider': () => {
    return `Ready to parse your technical rider!

Paste your rider text and I'll extract and organize:

• Audio Requirements
• Backline needs
• Monitor requirements
• Stage plot specifications
• Power requirements
• Hospitality requests

TIPS FOR BEST RESULTS:
• Include all sections of the rider
• PDF text copy works well
• Include any addendums or updates

Paste your rider content to begin!`;
  },

  'parse_gear': (ctx) => {
    if (ctx.gear.length === 0) {
      return `No gear in inventory yet. Add gear items to your inventory to get maintenance tracking and packing lists.

GEAR CATEGORIES SUPPORTED:
• Console & Processing
• Microphones
• DI Boxes & Preamps
• IEM Systems
...
Paste a gear list to import, or add items manually through the Gear screen.`;
    }

    return `CURRENT GEAR INVENTORY

Total Items: ${ctx.gear.length}

${ctx.gear.slice(0, 10).map(g => `• ${g.name} [${g.condition.toUpperCase()}]`).join('\n')}
${ctx.gear.length > 10 ? `\n... and ${ctx.gear.length - 10} more items` : ''}

To import more gear, paste a list in any of these formats:
• Line-by-line item names
• Tab-separated with details
• Spreadsheet export (CSV)

Paste your gear list to import!`;
  },

  'routing': (ctx) => {
    const tour = ctx.activeTour;
    if (!tour) return 'Create a tour with shows to get routing optimization suggestions.';

    return `TOUR ROUTING ANALYSIS
...
SCHEDULING
• Avoid back-to-back one-offs
• Build in off days after long drives (6+ hours)
• Consider time zones for crew adjustment

DRIVE TIME OPTIMIZATION
• Group regional shows together
• Consider routing direction (clockwise vs counter)
• Plan fuel and rest stops for crew

LOGISTICS
• Confirm load-in times accommodate drive distance
• Account for border crossings if international
• Pre-book hotels along routes

COST SAVING TIPS
• Minimize deadhead miles
• Consider fly dates for isolated markets
• Share transport with support acts when possible

Add shows to your tour to get specific routing suggestions!

Generated by Tour Flow`;
  }
};

// Helper to get mock response based on prompt type
function getMockResponse(prompt: string, context: Parameters<typeof MOCK_RESPONSES['day_sheet']>[0]): string {
  const promptLower = prompt.toLowerCase();

  if (promptLower.includes('day sheet')) return MOCK_RESPONSES['day_sheet'](context);
  if (promptLower.includes('advance')) return MOCK_RESPONSES['advance'](context);
  if (promptLower.includes('settlement')) return MOCK_RESPONSES['settlement'](context);
  if (promptLower.includes('mixing') || promptLower.includes('tips')) return MOCK_RESPONSES['mixing_tips'](context);
  if (promptLower.includes('rf') || promptLower.includes('frequency') || promptLower.includes('wireless')) return MOCK_RESPONSES['rf_coord'](context);
  if (promptLower.includes('maintenance') || promptLower.includes('checklist') || promptLower.includes('pre-tour')) return MOCK_RESPONSES['gear_check'](context);
  if (promptLower.includes('parse input') || promptLower.includes('input list')) return MOCK_RESPONSES['parse_input'](context);
  if (promptLower.includes('parse rider') || promptLower.includes('rider')) return MOCK_RESPONSES['parse_rider'](context);
  if (promptLower.includes('parse gear') || promptLower.includes('gear list')) return MOCK_RESPONSES['parse_gear'](context);
  if (promptLower.includes('routing') || promptLower.includes('optimize')) return MOCK_RESPONSES['routing'](context);

  // Default response for unrecognized prompts
  return `Thanks for your message! I'm Flow AI, your tour assistant.

I can help you with:
• Generating day sheets and advance documents
• Creating settlement templates
• Providing mixing tips and RF coordination advice
• Pre-tour maintenance checklists
• Parsing input lists, riders, and gear lists

Try one of the quick prompts above, or ask me about:
- "Generate a day sheet"
- "Give me mixing tips"
- "Help with RF coordination"
- "Create a pre-tour checklist"

Note: For full AI capabilities, add your OpenAI API key in the ENV tab.`;
}
...
`;
  ...
  const sendMessage = async (messageText?: string) => {
    ...
    // Build context for mock responses
    const mockContext = {
      activeTour: activeTour ? { name: activeTour.name, artist: activeTour.artist } : undefined,
      nextShow: nextShow ? {
        venue: nextShow.venue,
        city: nextShow.city,
        state: nextShow.state,
        date: nextShow.date,
        loadIn: nextShow.loadIn,
        soundcheck: nextShow.soundcheck,
        doors: nextShow.doors,
        showTime: nextShow.showTime,
        curfew: nextShow.curfew,
        capacity: nextShow.capacity,
        venueContact: nextShow.venueContact,
        venuePhone: nextShow.venuePhone,
      } : undefined,
      gear: gear.map(g => ({ name: g.name, condition: g.condition, notes: g.notes })),
      inputList: inputList.map(c => ({ channel: c.channel, source: c.source, mic: c.mic })),
    };
    ...
    if (!hasApiKey) {
      // Use mock responses when no API key
      await new Promise(resolve => setTimeout(resolve, 800)); // Simulate thinking delay

      const assistantContent = getMockResponse(text, mockContext);
    ...
    try {
      ...
      if (data.output_text) {
        ...
        // API error - fallback to mock response
        assistantContent = getMockResponse(text, mockContext);
      }
        ...
        <View className="px-5 pt-4 pb-3 flex-row items-center justify-between border-b border-white/5">
          <View className="flex-row items-center">
            ...
            <View>
              ...
              <Text className="text-gray-500 text-xs">Tour Assistant</Text>
            </View>
          </View>
